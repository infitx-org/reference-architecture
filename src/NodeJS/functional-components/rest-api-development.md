...
* API Snippets - api first approach that additionally splits the types into reusable snippets.
...
## API Snippets
This solution is largely the same as the API-first approach, where an OpenAPI definition is initially constructed, however the OpenAPI is automatically built by combining snippets of API definition. These snippets form the single source of truth for a defined type. The snippet of a type is then incorporated into all the OpenAPI definitions that use that type. Typescript definitions of a type are additionally constructed from the snippet that can be used in the typescript code using these API's.
